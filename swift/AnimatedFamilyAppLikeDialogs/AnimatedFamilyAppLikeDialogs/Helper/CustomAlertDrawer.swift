...
struct DrawerConfig {
    ...
    /// Limit access to only this file (Only set)
                    ...
                    ZStack {
                        if isPresented {
                            Rectangle()
                                ...
                                .transition(.opacity)
                                .onTapGesture {
                                    guard config.wrappedValue.isPresented else {
                                        return
                                    }
...
fileprivate struct AlertDrawerContent<Content: View>: View {
    ...
    var body: some View {
        ...
        VStack(spacing: 15) {
            ...
            HStack(spacing: 10) {
                ...
                .frame(height: config.sourceRect.height)
                /// This place the primary button above the secondary button
                .zIndex(1)
            }
            .buttonStyle(ScaledButtonStyle())
            .padding(.top, 10)
    ...
    private func fixedLocation(_ proxy: GeometryProxy) -> CGSize {
        ...
        return CGSize(
            width: isPresented ? 0 : (
                sourceRect.minX - proxy.frame(in: .global).minX
            ),
            height: isPresented ? 0 : (
                sourceRect.minY - proxy.frame(in: .global).minY
            )
        )
    }
}
